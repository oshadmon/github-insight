...
class SendData: 
   ...
   def check_row(self, table_name:str='github', timestamp:str='2000-01-01', repo:str='new repo', referral:str='github.com')->int: 
      """
      ...
      if table_name == 'github_referrals':
         ...
         self.cur.execute(stmt % (table_name, timestamp, repo, referral))
      else: 
         stmt="SELECT COUNT(*) FROM %s WHERE timestamp='%s' AND repo='%s';"
         self.cur.execute(stmt % (table_name, timestamp, repo)) 
   ...
   def insert_referrals(self):
      ...
      insert into 
      """
      inst_stmt="INSERT INTO github_referrals(row_id, timestamp, repo, referral, today) VALUES('%s', DATE('%s'), '%s', '%s', %s);"
      updt_stmt=("UPDATE github_referrals SET total=("
                   +"SELECT SUM(today) FROM github_referrals WHERE repo='%s' AND referral='%s' AND timestamp<='%s'"
                +") WHERE row_id='%s';")
      for obj in self.data['referrals']: 
         ...
         if self.check_row('github_referrals', timestamp, repo, referral) == 0: 
